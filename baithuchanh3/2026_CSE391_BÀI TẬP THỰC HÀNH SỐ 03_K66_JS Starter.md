...
>     **Trả lời:** JavaScript có thể chạy cả trên trình duyệt và trên
>     server (ví dụ Node.js).
> b) HTML, CSS, JavaScript mỗi phần chịu trách nhiệm chính về điều gì?
>
> -   HTML: Tạo cấu trúc và nội dung của trang web.\
> -   CSS: Dùng để định dạng, tạo kiểu và bố cục giao diện trang web.\
> -   JavaScript: Xử lý logic, tương tác và thay đổi nội dung trang web
>     động.
...
>   -   `number` là: Kiểu dữ liệu số (ví dụ 1, 2, 3, 3.14).\
>   -   `string` là: Kiểu dữ liệu chuỗi ký tự (ví dụ "Hello").\
>   -   `boolean` là: Kiểu dữ liệu logic chỉ có hai giá trị true hoặc
>         false.
---
...
>   **Trả lời:** Xin chào, mình là Hải Minh Nguyễn, năm nay mình 21 tuổi.

> - Nếu em quên dấu `;` hoặc quên dấu `+`, điều gì xảy ra? Trình duyệt
>   báo lỗi thế nào?
>   **Trả lời:** Nếu quên dấu `+` hoặc viết sai cú pháp, trình duyệt
>   sẽ báo lỗi trong Console như SyntaxError.


#### Bước 5: Phản tư nhanh (Reflection)

> - Điều thú vị nhất em vừa khám phá được về console là gì?
>   **Trả lời:** Có thể chạy thử nhanh code JavaScript và xem kết quả
>   ngay lập tức.
> - Em gặp lỗi cú pháp nào? Em đã xử lý bằng cách nào (tự sửa, hỏi bạn, đọc lỗi, tìm Google, …)?
>   **Trả lời:** Lỗi thiếu dấu ngoặc hoặc toán tử, em đọc lỗi trong
>   console và sửa lại.
---
...
> - Trường hợp `score = 9`: Dự đoán vs Thực tế: Giỏi -- Giống nhau
> - Trường hợp `score = 6`: Dự đoán vs Thực tế: Trung bình -- Giống nhau
...
>   Khi điểm số lớn hơn hoặc bằng 8.

> - Khi nào chương trình in `"Yếu"`?
>   Khi điểm nhỏ hơn 5.

> - Em hãy mô tả cấu trúc `if/else` bằng lời của em (có thể ví von “ngã rẽ” trong đời sống):
>   Cấu trúc if/else giống như một ngã rẽ. Nếu điều kiện đúng thì đi
>   theo nhánh if, nếu không đúng thì chuyển sang nhánh else hoặc else
>   if.
...
>   Phần điều kiện nhiều nhánh else if.

> - Em đã làm gì để vượt qua (thử nhiều lần, hỏi bạn, xem lại ví dụ, tra Google, …)?
>   Chạy thử nhiều giá trị khác nhau để hiểu cách hoạt động.
---
...
>   Nội dung đoạn văn status được thay đổi.
---
...
>   input xảy ra khi nội dung ô nhập thay đổi, click xảy ra khi nhấn
>   chuột.
...
>   DOM là mô hình biểu diễn trang HTML thành dạng cây để JavaScript
>   có thể truy cập và thay đổi các phần tử.
...
>   Đều xử lý sự kiện click để hiển thị thông báo.
...
>   jQuery dùng cú pháp ngắn gọn hơn như \$("#id") và .on().
